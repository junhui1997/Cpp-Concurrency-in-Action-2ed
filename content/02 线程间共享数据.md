...
void init() {
  if (!p) {  // 未上锁，其他线程可能在执行 #1，则此时 p 不为空，此处就直接跳过了if判断
  ...
  p->f();  // p 可能指向一块还未构造实例的内存，从而崩溃，这个问题出现在reset这个函数上，所以最好使用make_shared来分配指针
}
...
* static 局部变量在声明后就完成了初始化，这存在潜在的 race condition，如果多线程的控制流同时到达 static 局部变量的声明处，即使变量已在一个线程中初始化，其他线程并不知晓，仍会对其尝试初始化。为此，C++11 规定，如果 static 局部变量正在初始化，线程到达此处时，将等待其完成，从而避免了 race condition。只有一个全局实例时，可以直接用 static 而不需要 [std::call_once](https://en.cppreference.com/w/cpp/thread/call_once)，这个地方我理解的是自带了默认保护
